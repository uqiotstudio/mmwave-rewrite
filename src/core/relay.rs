...
impl<T: Clone + std::fmt::Debug> Relay<T> {
    /// Creates a new relay
    pub fn new() -> Self {
    ...
    pub fn register(&mut self, id: Id, destination: Destination) {
        debug!("Registering id {:?} to destination {:?}", id, destination);
        ...
        info!("Registered id {:?} to destination {:?}", id, destination);
    }
    ...
    pub fn deregister(&mut self, id: Id, destination: Destination) {
        debug!(
            "Deregistering destination {:?} from id {:?}",
            destination, id
        );
                ...
                for id in ids {
                    if let Some(channel) = self.channels.get(id) {
                        if channel.send(message.clone()).is_err() {
                            warn!("Failed to send message to id {:?}", id);
                        }
                    }
                }
            }
    ...
    #[instrument(skip_all)]
    pub fn subscribe(&self, id: Id) -> Option<broadcast::Receiver<T>> {
